...
void solve() {
	...
	int ans = 0;
	do {
		for (int l = K - 10; l <= K + 10; ++l)
			for (int r = l; r <= K + 10; ++r)
				for (int m = l; m <= r; ++m)
					ans = add(ans, (r - l) * 1ll * dp[K + l - x[ord[1]]] % MOD * dp[K + r - x[ord[3]]] % MOD * dp[K + m - x[ord[2]]] % MOD);

		cout << ans << "\n";

		for (int l = K - 10; l <= K + 10; ++l)
			for (int r = l; r <= K + 10; ++r) {
				ans = sub(ans, (r - l) * 1ll * dp[K + l - x[ord[1]]] % MOD * dp[K + l - x[ord[2]]] % MOD * dp[K + l - x[ord[3]]] % MOD); 
				ans = sub(ans, (r - l) * 1ll * dp[K + l - x[ord[1]]] % MOD * dp[K + r - x[ord[2]]] % MOD * dp[K + l - x[ord[3]]] % MOD); 
			}
		cout << ans << "\n";
		ans = 0;
