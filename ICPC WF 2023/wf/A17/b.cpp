...
const long double epsl = 1e-6;

bool inp[204][204], inp1[204][204];
...
template<class P> bool inPolygon(vector<P> &p, P a, bool strict = false){
	int cnt = 0, n = sz(p);
	for(int i = 0; i < n; i++){
		P q = p[(i + 1) % n];
		...
		cnt ^= cmp(((a.y < p[i].y) - (a.y < q.y)) * a.cross(p[i], q), 0) > 0;
	}
	return cnt;
}
...
Point<long double> get(Point<long double> s, Point<long double> e, int i1, int j1){
	...
	for(int i = 0; i < n; i++){
		...
		if(inter == a[i]){
			if((i1 != i && !inp[i1][i]) || (i == i1 && !inp1[i1][j1])) vec.pb(inter);
		} else if (inter == a[(i + 1) % n]){
			int j = (i + 1) % n;
			if((i1 != j && !inp[i1][j]) || (j == i1 && !inp1[i1][j1])) vec.pb(inter);
//			if(!inp[i1][(i + 1) % n]) vec.pb(inter);		
		}
		else vec.pb(inter);
	}
	assert(!vec.empty());
...
main() {
	...
	for (int i = 0; i < n; ++i)
		for (int j = 0; j < n; ++j) {
			inp[i][j] = inPolygon(a, a[j] + (a[j] - a[i])*epsl);
			inp1[i][j] = inPolygon(a, a[i] + (a[j] - a[i])*epsl);
//			cerr << i << ' ' << j << ' ' << inp[i][j] << endl;
		}

	for(int i = 0; i < n; i++){
		for(int j = i + 1; j < n; j++){
			Point<long double> s = a[i], e = a[j];
			Point<long double> e1 = get(s, e, i, j), s1 = get(e, s, j, i);
			bool ok = 1;
			if(cmp((e1 - s).dist(), (e - s).dist()) < 0){
				ok = 0;
				ans = max(ans, (e1 - s).dist());
			}
			if(cmp((s1 - e).dist(), (e - s).dist()) < 0){
				ok = 0;
				ans = max(ans, (e - s1).dist());
			}
			if(ok) ans = max(ans, (e1 - s1).dist());
		}
	}
