#include <bits/stdc++.h>                                           
#include <ext/pb_ds/assoc_container.hpp>
#include <ext/pb_ds/tree_policy.hpp>
 
using namespace std;
using namespace __gnu_pbds;
 
//#define int long long
//#pragma GCC optimize("Ofast")
//#pragma comment(linker, "/stack:200000000")
//#pragma GCC target("sse,sse2,sse3,ssse3,sse4")
 
 
#define file(s) freopen(s".in","r",stdin); freopen(s".out","w",stdout);
#define fastio ios_base::sync_with_stdio(0), cin.tie(0), cout.tie(0);
#define all(x) x.begin(), x.end()
#define sz(s) (int)s.size()
#define pb push_back
#define ppb pop_back
#define mp make_pair
#define s second
#define f first
 
 
 
 
typedef pair < long long, long long > pll;    
typedef pair < int, int > pii;  
typedef unsigned long long ull;         
typedef vector < pii > vpii;                                   	
typedef vector < int > vi;
typedef long double ldb;  
typedef long long ll;  
typedef double db;
 
typedef tree < int, null_type, less < int >, rb_tree_tag, tree_order_statistics_node_update > ordered_set;
 
const int inf = 1e9, maxn = 2e5 + 48, mod = 1e9 + 7, N = 2e5 + 12;
const int dx[] = {1, -1, 0, 0}, dy[] = {0, 0, 1, -1}, block = 300;
const pii base = mp(1171, 3307), Mod = mp(1e9 + 7, 1e9 + 9);
const db eps = 1e-12, pi = acos(-1);
const ll INF = 1e18;


int n, m, nxt[N], prv[N], t[N << 2][2];
int mx[N], mn[N], p[N]; 
queue < pii > Q;
set < int > q[N];

void update(int pos, int v = 1, int tl = 1, int tr = n) {
	if (tl == tr) {
		t[v][0] = prv[pos];
		t[v][1] = nxt[pos];
		return;
	}

	int tm = (tl + tr) >> 1;
	if (pos <= tm)
		update(pos, v << 1, tl, tm);
	else
		update(pos, v << 1 | 1, tm + 1, tr);	

	t[v][0] = min(t[v << 1][0], t[v << 1 | 1][0]);
	t[v][1] = max(t[v << 1][1], t[v << 1 | 1][1]);
}

int get(int l, int r, int v = 1, int tl = 1, int tr = n) {
	if (l > r || tl > r || l > tr || (t[v][0] >= l && t[v][1] <= r))
		return -1;

	if (tl == tr)
		return tl;

	int tm = (tl + tr) >> 1;
	if (l <= tl && tr <= r) {
		if (t[v << 1][0] < l || t[v << 1][1] > r)
			return get(l, r, v << 1, tl, tm);
		return get(l, r, v << 1 | 1, tm + 1, tr);
	}

	int res = get(l, r, v << 1, tl, tm);
	return res == -1 ? get(l, r, v << 1 | 1, tm + 1, tr) : res;
}
int get(int v) {
	return p[v] == v ? v : p[v] = get(p[v]);
}

inline void upd(int v) {
	int u = get(v);

	auto it = q[u].lower_bound(v);
	if (it == q[u].begin())
		prv[v] = inf;
	else {
		--it;
		prv[v] = *it;
		++it;
	}		

	++it;
	nxt[v] = it == q[u].end() ? -inf : *it; 

	if (prv[v] < inf) {
		nxt[prv[v]] = v;
		update(prv[v]);
	}

	if (nxt[v] > 0) {
		prv[nxt[v]] = v;
		update(nxt[v]);
	}

	update(v);

	if (prv[v] < inf) 
		Q.push({prv[v], v});

	if (nxt[v] > 0) 
		Q.push({v, nxt[v]});
}


void merge(int v, int u) {
	v = get(v), u = get(u);
	if (v == u)
		return;

	if (mn[v] > mn[u])
		swap(v, u);

	for (auto x : q[u])
		q[v].insert(x);

	q[u].clear();
	p[u] = v;	

	upd(mx[v]);
	upd(mn[v]);
	upd(mx[u]);
	upd(mn[u]);

	mx[v] = max(mx[v], mx[u]);
	mn[v] = min(mn[v], mn[u]);	

	while (sz(Q)) {
		int x = Q.front().f, y = Q.front().s;
		Q.pop();

		int z = get(x +  1, y - 1);
		if (z != -1 && get(v) != get(z)) 
			merge(v, z);
	}
}



main () {
	scanf("%d%d", &n, &m);
	for (int i = 1; i <= n; ++i) {
		p[i] = i;
		q[i].insert(i);
		nxt[i] = -inf;
		prv[i] = inf;
		mx[i] = mn[i] = i;
		update(i);
	}

	while (m--) {
		int e, v, u;
		scanf("%d%d%d", &e, &v, &u);
		if (e == 1)
			merge(v, u);
		else 
			printf("%d", get(v) == get(u));						
	}
}